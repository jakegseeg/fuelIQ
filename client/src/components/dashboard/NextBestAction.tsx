...
export function NextBestAction({ date, day, workout }: Props) {
  ...
  if (!hasLoggedFood) {
    return (
      <section className="overflow-hidden rounded-card bg-surface ring-1 ring-inset ring-ink-200" aria-labelledby="next-step-title">
        <div className="flex items-center gap-4 p-4 sm:p-5">
          <span className="flex h-11 w-11 flex-none items-center justify-center rounded-full bg-accent-100 text-accent-500"><Plus size={22} aria-hidden /></span>
          ...
          <button type="button" onClick={() => navigate('/log')} className="btn-primary flex-none px-3 sm:px-4" aria-label="Log your first meal">
  ...
  if (workout && !isRestDay(workout)) {
    ...
    return (
      <section className="overflow-hidden rounded-card bg-surface ring-1 ring-inset ring-ink-200" aria-labelledby="next-step-title">
        <div className="flex items-center gap-4 p-4 sm:p-5">
          <span className="wellness-emphasis-icon"><Dumbbell size={21} aria-hidden /></span>
          <div className="min-w-0 flex-1">
            <p className="section-label">Your next step</p>
          ...
          <button type="button" onClick={start} className="btn-emphasis flex-none px-3 sm:px-4"><span className="hidden sm:inline">Start</span><ChevronRight size={20} aria-hidden /></button>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="overflow-hidden rounded-card bg-surface ring-1 ring-inset ring-ink-200" aria-labelledby="next-step-title">
      <div className="flex items-center gap-4 p-4 sm:p-5">
        ...
        <button type="button" onClick={() => navigate('/log')} className="btn-ghost flex-none px-3 sm:px-4" aria-label="Open food log"><span className="hidden sm:inline">View log</span><ChevronRight size={20} aria-hidden /></button>
      </div>
      <div className="flex items-center gap-2 border-t border-ink-200 px-4 py-2.5 text-[13px] text-ink-600"><CheckCircle2 size={16} className="text-accent-500" aria-hidden /> Recovery is part of the plan—today may be a rest day.</div>
    </section>
  );
}
