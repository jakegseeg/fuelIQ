...
export function Segmented<T extends string>({
  ...
  return (
    <div className="inline-flex min-h-11 rounded-xl bg-ink-100/45 p-1 ring-1 ring-inset ring-ink-200" role="group">
      {options.map((opt) => (
        <button
          key={opt.value}
          type="button"
          ...
          className={`min-h-9 rounded-lg font-medium transition ${
...
export function SelectCard({
  ...
  return (
    <button
      ...
      className={`flex min-h-16 w-full items-center gap-4 rounded-xl border p-4 text-left transition ${
        selected
          ? 'state-accent'
