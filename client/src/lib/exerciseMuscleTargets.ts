...
export function resolveMuscleTargetLabel(exercise: PlanExercise): string | undefined {
  const fromLookup = resolveByGroupAndName(exercise.name, exercise.rotationGroup);
  if (fromLookup) return fromLookup;

  const wger = exercise.muscleGroups?.[0];
  if (wger) return fallbackLabel(wger);
